...
CString NombreAlumno1 = "Yeray";
CString ApellidosAlumno1 = "Cordero Carrasco";
CString NIUAlumno1 = "1599053"; // NIU alumno1

// No rellenar en caso de grupo de un alumno
CString NombreAlumno2 = "Laia";
CString ApellidosAlumno2 = "Rubio Castro";
CString NIUAlumno2 = "1600830"; // NIU alumno2
...
bool EjecutarLineaDeComandos(int nArgs, CString args[])
{
	...
	if (nArgs == 1 && args[0].CompareNoCase("Authors") == 0) {
		...
		if (!NIUAlumno2.IsEmpty()) file << NIUAlumno2 << endl;
		file.close();
		return true;
	}
	if (nArgs < 3) {
	...
	if (args[1].CompareNoCase("Prim") == 0) {
	...
	else if (args[1].CompareNoCase("DynamicProgramming") == 0) {
		if (nArgs != 5) {
			cout << "Número de argumentos erroneo" << endl;
			return true;
		}
		...
		try {
			...
			visits.Load(args[3]);
			double t0 = Clock();
			CTrack track = SalesmanTrackDynamicProgramming(graph, visits);
			double t1 = Clock();
			track.Save(args[4]);
			cout << endl << "TIEMPO DE EJECUCION: " << fixed << setprecision(9) << t1 - t0 << endl;
		}
