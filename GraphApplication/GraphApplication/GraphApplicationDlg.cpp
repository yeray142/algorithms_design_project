...
// GraphApplicationDlg.cpp: archivo de implementaci�n
//
...
// Cuadro de di�logo de CGraphApplicationDlg
...
BEGIN_MESSAGE_MAP(CGraphApplicationDlg, CDialogEx)
	...
	ON_BN_CLICKED(IDC_BUTTON_DYNAMIC_PROGRAMMING, &CGraphApplicationDlg::OnBnClickedButtonDynamicProgramming)
END_MESSAGE_MAP()
...
// OnBnClickedButtonDynamicProgramming =========================================

void CGraphApplicationDlg::OnBnClickedButtonDynamicProgramming()
{
	if (m_Graph.GetNVertices() == 0) {
		AfxMessageBox("El algoritmo de programaci�n din�mica necesita un grafo para ejecutarse", MB_OK | MB_ICONERROR);
		return;
	}
	if (m_Visits.m_Vertices.empty()) {
		AfxMessageBox("El algoritmo de programaci�n din�mica necesita unas visitias para ejecutarse", MB_OK | MB_ICONERROR);
		return;
	}
	PrintLog("OnBnClickedButtonDynamicProgramming");
	try {
		m_Graph.SetDistancesToEdgeLength();
		cout << "SALESMAN DYNAMIC PROGRAMMING" << endl;
		double t0 = Clock();
		m_Track = SalesmanTrackDynamicProgramming(m_Graph, m_Visits);
		double t = Clock() - t0;
		cout << "TRACK: " << m_Track << endl;
		cout << "TRACK LENGTH: " << m_Track.Length() << endl;
		cout << "Run time: " << t << "seg." << endl;
